...
def get_help(helping=""):
    ...
    if helping and helping not in (con.POSSIBLE_HELP + con.HIDDEN_HELP + var.USERS + var.COMMANDS):
        ...
        if con.POSSIBLE_HELP:
            help("HELP_POS_HELP_TOPICS", form=[", ".join(con.POSSIBLE_HELP), "s" if len(con.POSSIBLE_HELP) > 1 else ""])
            if (con.HIDDEN_HELP and var.DEBUG_MODE) or (con.HIDDEN_HELP and var.SHOW_HIDDEN_HELP):
                help("HELP_HIDDEN_COMMANDS", form=[", ".join(con.HIDDEN_HELP), "s" if len(con.HIDDEN_HELP) > 1 else ""])
        ...
        if con.FIRST_DEV:
            help("HELP_FIRST_DEV", form=[", ".join(con.FIRST_DEV), "s" if len(con.FIRST_DEV) >1 else ""])
        if con.USER_HELP:
            help("HELP_USER_HELPING", form=["s" if len(con.USER_HELP) > 1 else "", ", ".join(con.USER_HELP)])
        if con.CODERS:
            help("HELP_CODERS", form=["s" if len(con.CODERS) > 1 else "", ", ".join(con.CODERS)])
            if con.GUI_CODERS:
        ...
        if con.BETA_TESTERS:
            help("HELP_BETA_TESTERS", form=["s" if len(con.BETA_TESTERS) > 1 else "", ", ".join(con.BETA_TESTERS)])
        if con.TRANSLATORS:
            help("HELP_TRANSLATORS", form=["s" if len(con.TRANSLATORS) > 1 else "", ", ".join(con.TRANSLATORS)])
            if con.FRENCH_TRANSLATORS:
                help("HELP_FRENCH_TRANSLATORS", form=["s" if len(con.FRENCH_TRANSLATORS) > 1 else "", ", ".join(con.FRENCH_TRANSLATORS)])
        if con.OTHER_SUPPORT:
            help("HELP_OTHER_SUPPORT", form=["s" if len(con.OTHER_SUPPORT) > 1 else "", ", ".join(con.OTHER_SUPPORT)])
        if con.SPECIAL_THANKS:
            help("HELP_SPECIAL_THANKS", form=", ".join(con.SPECIAL_THANKS))
        if con.EXT_HELP:
            help("HELP_EXT_HELP", form=["s" if len(con.EXT_HELP) > 1 else "", ", ".join(con.EXT_HELP)])
        if con.EMAIL:
            help("HELP_EMAIL", form=[con.EMAIL, con.PROGRAM_NAME])
        if con.POSSIBLE_HELP:
            help("", "HELP_POSSIBLE_HELP", form=[", ".join(con.POSSIBLE_HELP), "s" if len(con.POSSIBLE_HELP) > 1 else ""])
            if (con.HIDDEN_HELP and var.DEBUG_MODE) or (con.HIDDEN_HELP and var.SHOW_HIDDEN_HELP):
                help("HELP_HIDDEN_HELP", form=[", ".join(con.HIDDEN_HELP), "s" if len(con.HIDDEN_HELP) > 1 else ""])
